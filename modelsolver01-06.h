 ...
 * 2. 负责计算软件定义的 36 个试井模型理论图版数据。
 * 3. 涵盖三种储层组合的数学求解:
 * - 夹层型 + 夹层型 (Model 1-12)
 * - 夹层型 + 均质   (Model 13-24)
 * - 径向复合 (均质+均质) (Model 25-36)
 * 4. 每个储层组合下包含 3 种边界(无限大、封闭、定压)和 4 种井筒储集类型(定井储、线源解、Fair、Hegeman)。
 * 5. 提供了基于 Stehfest 数值反演算法和边界元方法 (BEM) 的核心求解流程定义。
 * 6. 支持变井储效应在 Laplace 域内的强耦合运算，提升计算稳健性。
 */
